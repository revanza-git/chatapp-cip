...
function CreateDocumentDialog({
  ...
  const handleFileUpload = (data: {
    ...
    setFormData(prev => ({
      ...
      content: prev.content || `[File uploaded: ${data.fileName}]\n\nPlease add document content here or the file content will be referenced.`,
      description: prev.description || `Document uploaded from ${data.fileName} (${(data.fileSize / 1024).toFixed(2)} KB)`,
      file_path: data.fileId, // Store the fileId as file_path for backend reference
      category: prev.category || 'General', // Set default category if empty
    }));
    
    // Show a message to the user
    alert('File uploaded successfully! Please fill in the remaining fields (especially Category) before creating the document.');
  };
